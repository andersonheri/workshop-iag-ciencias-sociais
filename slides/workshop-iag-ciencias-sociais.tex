...
\institute{}
\date{2026}
\titlegraphic{\includegraphics[width=1.5cm]{assets/cem_logo_trim.png}}
% =====================================================================
%  DOCUMENTO
% =====================================================================
\begin{document}
...
\end{lstlisting}
\centering\small Copie, preencha os colchetes e ajuste. É o Modelo 00 do Manual de Sobrevivência.
\end{frame}
...
\begin{frame}{O mesmo padrão para questões de avaliação}
  \begin{block}{Adapte a mesma estrutura}
    \small Papel $\rightarrow$ tarefa $\rightarrow$ contexto $\rightarrow$ formato, agora para gerar 5 questões (2 dissertativas, 2 de múltipla escolha, 1 de caso curto).
  \end{block}
  \vfill
  \begin{itemize}
    \item Cada questão com objetivo de aprendizagem (Bloom), tempo estimado e gabarito comentado.
    \item \alert{A mesma instrução negativa}: não invente dado ou fato, use só conceitos da literatura.
    \item Peça também uma tabela de especificação (questão $\times$ objetivo $\times$ peso).
  \end{itemize}
  \vfill
  \centering\scriptsize Modelo completo, pronto para copiar, no Manual de Sobrevivência.
\end{frame}
...
\begin{frame}{Escolha e teste a ferramenta certa (Hands-on 3)}
  \begin{block}{Escolha uma tarefa real de pesquisa sua}
    Buscar literatura sobre um tema, ler um PDF longo ou mapear as citações de um trabalho central.
  \end{block}
  \vfill
  \begin{enumerate}
    \item Na tabela anterior, escolha a ferramenta pela \textbf{tarefa}, não pela que você já conhece.
    \item Rode a busca ou o resumo e confira se cada afirmação vem com fonte rastreável.
    \item A ferramenta não citou a fonte? Desconfie: é sinal de Linhagem 2 sem ancoragem.
  \end{enumerate}
\end{frame}

\begin{frame}{O esforço certo do modelo}
  \small
  \begin{itemize}
    \item A maioria dos provedores oferece níveis de esforço: rápido para tarefas simples, raciocínio estendido para tarefas com várias etapas.
    \item Mais ``pensamento'' nem sempre é melhor: gasta tokens sem ganhar acurácia (\textit{overthinking}, Chen et al., 2025).
    \item Além de um certo limiar, o mesmo tipo de modelo tende a \textbf{piorar} (Shojaee et al., 2025).
  \end{itemize}
  \pause
  \vfill
  \scriptsize
  ...
  \begin{tabular}{p{4.0cm} p{4.0cm} p{4.0cm}}
    ...
    \midrule
    Revisão ortográfica; traduzir parágrafo curto; pergunta factual simples
    & Fichamento padrão; formatar ABNT; rascunho de e-mail formal
    & Desenho metodológico; síntese comparativa; depurar erro não óbvio \\
    \bottomrule
  \end{tabular}
  \pause
  \vfill
  \centering\small Resposta objetiva e fácil de checar? Baixo esforço resolve.
  Exige ponderar \textit{trade-offs} ou encadear etapas? Vale o modelo avançado.
\end{frame}

\begin{frame}{Ferramentas, tokens e custo: qual usar em cada situação}
  \scriptsize
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular}{p{2.6cm} p{3.5cm} p{3.6cm} p{3.7cm}}
    \toprule
    \textbf{Critério} & \textbf{Chat (claude.ai)} & \textbf{Cowork} & \textbf{Claude Code} \\
    \midrule
    Arquivos locais & Só upload manual & Acessa uma pasta local & Acesso total ao ambiente \\
    ...
    Consumo de tokens & Por mensagem & Por tarefa, incluso na assinatura & Por ação, cobrado à parte na API \\
    \bottomrule
  \end{tabular}
  \pause
  \vfill
  \begin{exampleblock}{Se você usa ChatGPT em vez do Claude}
    \small O \textbf{Work} ocupa o lugar do Cowork; o \textbf{Codex} ocupa o
    lugar do Claude Code. O critério de escolha desta tabela vale para as
    duas plataformas.
  \end{exampleblock}
  \pause
  \centering\small Dúvida pontual? Chat. Pasta inteira para organizar?
  Cowork. Pipeline de dados de ponta a ponta? Claude Code.
\end{frame}
...
\begin{frame}{Fichamento comparativo (Hands-on 4)}
...
\begin{frame}{Melhore um parágrafo seu (Hands-on 5)}
...
\begin{lstlisting}
Você é um tradutor acadêmico, especializado em inglês
científico para a área de [área].
...
\end{lstlisting}
\centering\small O Manual de Sobrevivência traz mais três modelos deste eixo: projeto para agência de fomento, carta de motivação (BEPE/PDSE) e parecer \textit{ad hoc} como revisor.
\end{frame}
...
\begin{frame}{Escreva sua declaração de uso (Hands-on 6)}
  \begin{block}{}
...
\begin{frame}{Registre suas decisões: o histórico de prompts}
  \begin{exampleblock}{Modelo de entrada para o seu registro de decisões}
    \small \textbf{Data e hora} $\bullet$ \textbf{Ferramenta e modelo} $\bullet$ \textbf{Tarefa} $\bullet$ \textbf{Prompt} (arquivo separado) $\bullet$ \textbf{Decisão} (integral / com edição / rejeitado) $\bullet$ \textbf{Justificativa}
  \end{exampleblock}
  \pause
  \vfill
  \footnotesize
  \begin{itemize}
    \item A declaração resume ao final; o registro documenta cada decisão, enquanto o trabalho acontece.
    \item Guarde o prompt num arquivo à parte (pasta \texttt{prompts/}) e referencie o nome no registro.
    \item Registre só os pontos de decisão, não cada mensagem: o que influenciou dado, análise ou texto.
    \item \textbf{Via mais forte}: com IA agêntica (Claude Code, Cowork), o agente escreve direto no arquivo, sem copiar e colar.
  \end{itemize}
\end{frame}
...
\begin{frame}{Rode uma tarefa curta com um agente (Hands-on 7)}
  \begin{block}{Escolha uma tarefa pequena e bem definida}
    Organizar os arquivos de uma pasta de projeto, gerar uma planilha a partir de dados brutos, ou pedir a um agente que gere e teste um trecho curto de código.
  \end{block}
  \vfill
  \begin{enumerate}
    \item Descreva o \textbf{objetivo final}, não o passo a passo. Deixe o agente planejar.
    \item Acompanhe: ele parou para perguntar quando havia ambiguidade?
    \item Ao final, avalie só o resultado. Você conseguiria explicar o que ele fez?
  \end{enumerate}
\end{frame}
...
\begin{frame}{Configure suas instruções personalizadas}
  \small
  \begin{block}{O que é}
    Um texto fixo que vale em toda conversa nova, sem repetir a cada
    prompt. No Claude: Configurações $\rightarrow$ Perfil
    $\rightarrow$ ``Instruções personalizadas'' (vale para chat e Cowork).
  \end{block}
  \pause
  \vfill
  \begin{itemize}
    \item \textbf{Padrão persona} (White et al., 2023): um papel fixo muda o vocabulário, o nível de detalhe e o critério de julgamento da resposta.
    \item \textbf{Contexto bem colocado} (Reynolds e McDonell, 2021): boa parte do que parece ``mágica'' no desempenho vem da instrução, não da tarefa isolada.
  \end{itemize}
  \pause
  \vfill
  \begin{alertblock}{}
    \centering Instruções personalizadas aplicam esse princípio de forma
    permanente, em vez de reescrevê-lo prompt a prompt.
  \end{alertblock}
\end{frame}

\begin{frame}{O que colocar nas suas instruções}
  \small
  \begin{itemize}
    \item[$\square$] Quem sou eu, e para quem eu escrevo?
    \item[$\square$] Que formato de resposta eu prefiro?
    \item[$\square$] Que postura eu espero do assistente? (crítica, não
    validação automática)
    \item[$\square$] O que eu explicitamente não quero ver?
  \end{itemize}
  \pause
  \vfill
  \begin{exampleblock}{}
    \small Escreva como uma orientação curta para um assistente novo:
    direta, com exemplos do que é ou não aceitável.
  \end{exampleblock}
  \pause
  \vfill
  \begin{alertblock}{O que isso não faz}
    \small\centering Não substitui o contexto de cada tarefa, nem tem
    prioridade sobre instruções de \textit{Projects}.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Novidade, a marca d'água invisível}
  ...
  \begin{block}{O que é}
    \begin{itemize}
      \item Modelos Claude lançados a partir de 2 de agosto de 2026 embutem marca estatística imperceptível em todo texto gerado.
      \item Imagens em SVG, PNG e JPG recebem metadados C2PA assinados.
      \item Resposta ao Artigo 50 do AI Act europeu.
    \end{itemize}
  \end{block}
  \pause
  \vspace{0.1cm}
  \begin{alertblock}{O limite que importa}
    \begin{itemize}
      \item A marca \textbf{não prova autoria}: texto escrito por você e só revisado pela IA pode carregá-la.
      \item A \textbf{ausência} da marca também não prova o contrário: edição extensa e paráfrase reduzem a detecção.
    \end{itemize}
  \end{alertblock}
  \pause
...
\begin{frame}{Como pensar sobre isso}
  ...
  \begin{itemize}
    ...
    \pause
    \item \textbf{Onde aprender}, documentação oficial das ferramentas, Anthropic Academy, tutoriais em vídeo. O Manual de Sobrevivência tem os links completos.
  \end{itemize}
\end{frame}
...
\begin{frame}{Sua declaração de uso, retomando}
  ...
  \vfill
  \begin{itemize}
    \item Se você já escreveu a sua no Hands-on 6, guarde-a junto ao trabalho em andamento.
...
\begin{frame}{Recursos para continuar}
  \small
  \begin{itemize}
    \item \textbf{Manual de Sobrevivência do Cientista Social com IA}, o guia de bolso deste curso: os 29 modelos de prompt, organizados no ciclo Aprenda $\rightarrow$ Configure $\rightarrow$ Revise $\rightarrow$ Consolide, checklists por perfil e bibliografia comentada por trilha de estudo.
...
\begin{frame}{Referências novas do Dia 2}
  ...
  \begin{itemize}
    ...
    \item WHITE, J. et al. A Prompt Pattern Catalog to Enhance Prompt Engineering with ChatGPT. Preprint: arXiv:2302.11382, 2023.
    \item REYNOLDS, L.; McDONELL, K. Prompt Programming for Large Language Models: Beyond the Few-Shot Paradigm. In: Extended Abstracts of the 2021 CHI Conference on Human Factors in Computing Systems. New York: ACM, 2021. Art. 314. DOI: 10.1145/3411763.3451760.
  \end{itemize}
\end{frame}
